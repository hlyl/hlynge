...
echo RootDirectory is %rootDirectory%
set workDrive=
set prefixLinkRoot=
...
echo Link going to %rootDirectory%%prefixLinkRoot%\
...
for /f "tokens=*" %%D in ('dir /b "%rootDirectory%\Missions\"') do (
    ...
    IF exist "%gameDirectory%\Missions\!missionFolder!\" (
        ...
        rmdir /s /q "%gameDirectory%\Missions\!missionFolder!\"
    ) else (
    ...
    IF exist "%serverDirectory%\MPMissions\!missionFolder!\" (
        ...
        rmdir /s /q "%serverDirectory%\MPMissions\!missionFolder!\"
    ) else (
