@echo off
echo Now we are binarizing the pbo
cd /D "%~dp0"
...
for /f "delims=" %%a in ('call ExtractData.bat ../../%modPath%/project.cfg ../../user.cfg KeyDirectory') do (
	set keyDirectory=%%a
)

for /f "delims=" %%a in ('call ExtractData.bat ../../%modPath%/project.cfg ../../user.cfg KeyName') do (
	set keyName=%%a
)

for /f "delims=" %%a in ('call ExtractData.bat ../../%modPath%/project.cfg ../../user.cfg ToolsDirectory') do (
	set dayzToolsPath=%%a
)
